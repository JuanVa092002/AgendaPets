...
const servicios=()=>JSON.parse(localStorage.getItem(KEY_S)||"[]").filter(s=>s.visible!==false).map(s=>({
    ...s,
    id:Number(s.id),
...
const etiqueta=()=>{
    const e=elegidos();
    return e.length>1?`${e[0].nombre} +${e.length-1}`:(e[0]?.nombre||"");
};
...
const avis=(titulo,texto,icon="warning")=>Swal.fire({
    title:titulo,
    text:texto,
    icon,
...
const clock=(h)=>{
    ...
    return`${String(n).padStart(2,"0")}.00 ${n<12?"AM":"PM"}`;
};

const rango=(h)=>{
    const n=Number(h.slice(0,2));
    return`${n%12||12} ${n<12?"AM":"PM"} - ${(n+1)%12||12} ${(n+1)<12?"AM":"PM"}`;
};

const ampm=(h)=>{
    const[H,M]=h.split(":");
    const n=Number(H);
    return`${n%12||12}:${M} ${n<12?"a. m.":"p. m."}`;
};
...
const mascota=()=>({
    nombre:$("mascota-nombre").value.trim(),
    tipo:$("mascota-tipo").value,
    raza:$("mascota-raza").value.trim(),
    tamano:$("mascota-tamano").value,
    dueno:$("dueno-nombre").value.trim(),
    telefono:$("dueno-telefono").value.trim(),
    notas:$("mascota-notas").value.trim()
});
...
function pintarPick(){
    const n=estado.ids.length;
    const bits=[
        n?`${n} servicio${n>1?"s":""} · ${money(total())}`:"Elige un servicio"
    ];
    if(estado.fecha&&estado.hora){
        bits.push(
            `${estado.fecha.slice(8)} ${MESES[Number(estado.fecha.slice(5,7))-1].slice(0,3)} · ${ampm(estado.hora)}`
        );
    }
...
function pintarServicios(){
    ...
    $("lista-servicios").innerHTML=lista.length?lista.map(s=>`
                <button
                    type="button"
                    class="reserva-card ${
                        estado.ids.includes(Number(s.id))
                            ? "is-selected"
                            : ""
                    }"
                    data-id="${s.id}">
                    ...
                    <span>
                        ...
                        <small>${s.duracion||""}</small>
                    </span>

                </button>
            `).join(""):`
                <p class="lead">
                    Aún no hay servicios en el panel de la peluquería.
                </p>
            `;
    pintarPick();
}
...
function pintarFecha(){
    ...
    for(let f=0;f<6;f++){
        ...
        for(let c=0;c<7;c++){
            if(d<1||d>tot){
                ...
                const past=iso<hoy||new Date(iso+"T00:00:00").getDay()===0;
    ...
    const man=libres.filter(hr=>hr<"13:00");
    const tar=libres.filter(hr=>hr>="13:00");
    ...
    $("lista-horas").innerHTML=libres.length?grupo("Mañana",man)+grupo("Tarde",tar):`
                <p class="lead">
...
function pintarGrilla(){
    const ini=estado.semanaInicio;

    const q=($("buscar-cita").value||"")
        .trim()
        .toLowerCase();

    const dias=[...Array(6)].map((_,i)=>{
    ...
    let h=`
        <div class="agenda-grid__cell is-head">
            GMT-5
        </div>
    `;
    ...
    HORAS.forEach(hora=>{
        h+=`
            <div class="agenda-grid__cell is-time ${
                hora===estado.hora||
                (!estado.hora&&hora==="10:00")
                    ? "is-now"
                    : ""
            }">
        ...
        dias.forEach(d=>{
            const iso=ymd(d);
            const oc=ocupada(iso,hora);
            const ok=abierta(iso,hora)&&iso>=hoy;
            const sel=estado.fecha===iso&&estado.hora===hora;

            const hide=q&&oc&&!`${oc.servicio||""} ${oc.mascota||""}`
                .toLowerCase()
                .includes(q);
            ...
            const txt=(!hide&&oc)
                ?`${oc.servicio||""}<br>${rango(hora)}`
                :sel
                    ?`${etiqueta()}<br>${rango(hora)}`
                    :"";
...
// ======================================================
// SNAPSHOT PARA EDITAR ANTES DE CONFIRMAR
// ======================================================
...
function pintarResumen(){
    ...
    const[y,mo,d]=estado.fecha.split("-");

    const diaNom=DIA_NOM[
        new Date(
            estado.fecha+"T00:00:00"
        ).getDay()
    ];

    const inicial=(m.nombre||"?")
        .charAt(0)
        .toUpperCase();

    const serviciosHtml=elegidos()
        .map(s=>`
                <li>
    ...
    $("resumen-cita").innerHTML=`
        ...
        <p class="revision-hint">
            ...
            Puedes ajustar servicios, datos o fecha antes de confirmar.
        </p>
        ...
        <article
            ...
            <div class="revision-card__head">
                ...
                <span class="revision-card__icon">
        ...
        <article
            ...
            <div class="revision-card__head">
                ...
                <span class="revision-card__avatar">
                    ${inicial}
                </span>
        ...
        <article
            class="revision-card revision-card--fecha"
            data-edit="3"
            role="button"
            tabindex="0">
            ...
            <div class="revision-card__head">
                ...
                <span class="revision-card__icon revision-card__icon--cal">
                ...
                <div class="revision-card__copy">
                    ...
                    <h2>
                        ${diaNom}, ${d} de ${MESES[Number(mo)-1]}
                    </h2>

                    <p>
                        ${ampm(estado.hora)} · GMT-5
                    </p>
...
function mostrarPaso(p){
    ...
    document
        .querySelectorAll(".reserva-panel")
        .forEach(el=>
            el.classList.toggle(
                "d-none",
                Number(el.dataset.panel)!==p
            )
        );

    document
        .querySelectorAll(".reserva-stepper li")
        .forEach(el=>{
    ...
    $("btn-atras").textContent=estado.paso===1
        ?"Atrás"
        :"Atrás";
...
function validar(){
    ...
    if(
        ...
        if(m.nombre.length<2){
            return"Indica el nombre de tu mascota.";
        }

        if(!m.tipo||!m.tamano){
            return"Completa tipo y tamaño.";
        }
    ...
    if(
        (estado.paso===3||
            estado.paso===4)&&
        (!estado.fecha||!estado.hora)
    ){
        return"Elige un día y un horario libre.";
    }
...
$("lista-servicios").onclick=e=>{
    const c=e.target.closest(".reserva-card");
    if(!c)return;

    const id=Number(c.dataset.id);

    estado.ids=estado.ids.includes(id)
        ?estado.ids.filter(x=>x!==id)
        :estado.ids.concat(id);
...
function guardarReserva(usuario){
    ...
    const citasActuales=obtenerTodasLasCitas();
    ...
    Swal.fire({
        ...
        text:
            `La cita de ${m.nombre} fue reservada correctamente.`,
...
$("mini-calendario").onclick=e=>{
    ...
    if(m){
        estado.mesVista=new Date(
            estado.mesVista.getFullYear(),
            estado.mesVista.getMonth()+
                Number(m.dataset.mes),
            1
        );
...
$("grilla-semana").onclick=e=>{
    ...
    estado.fecha=s.dataset.fecha;
    estado.hora=s.dataset.hora;
...
$("semana-prev").onclick=()=>{
    estado.semanaInicio.setDate(
        estado.semanaInicio.getDate()-7
    );
